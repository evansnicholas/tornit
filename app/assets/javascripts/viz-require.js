    ...
    return { 
        displayDtsGraph: function( json, graphContainer ) {
            ...
            node.append("text")
                ...
                .on('click', function(d){ console.log(d.uri); this.showTaxonomyDocument( d.uri ); });
            
        },

        showTaxonomyDocument: function( docUri ) {
            $("#content").load('/assets/html/taxonomy-document-viewer.html', function() {
                this.doForSelectedEntrypoint(function( uri ){ this.getTaxoDoc(uri, docUri); });
            });
        },
        ...
        displayPresentationTree: function( elrJson, container ){
            ...
            var totalConcepts = this.positionConcepts(elrJson.roots, 0, 0, positionedConcepts);
            ...
            node.append("text")
                ...
                .each(function(d) { 
                    var labelHtml = this.formatLabelPopoverHtml(d.concept.labels);
        ...
        positionConcepts: function( concepts, depth, totalSeenConcepts, accumulatedConcepts ) {
            function positionConcept( memo, element, index, list){
                ...
                var totalSeenConcepts = this.positionConcepts(element.children, depth + 1, memo + 1, accumulatedConcepts);
        ...
        displayENameHierarchy: function( hierarchy, container ) {
            ...
            var positionedENames = this.positionHierarchicalENames(hierarchy);
